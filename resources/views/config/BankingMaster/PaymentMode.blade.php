        ...
        <form id='AddForm' method="post" autocomplete="off">
            <div class="row justify-content-center">
                ...
                <div class="col-auto">
                    ...
                    <a class="btn btn-success btn-icon" href="{{ url('paymentMode/add') }}" title="Create">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                    ...
                    <a class="btn btn-light btn-icon" href="{{ url('paymentMode_report/excel') }}" target="_blank" title="Excel">
                    ...
                    <a class="btn btn-light btn-icon" href="{{ url('paymentMode_report/pdf') }}" target="_blank" title="Pdf">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px">
                            <path fill="#e53935"
...
@if ($action == 'add' || $action == 'edit')
    <nav id="navbar-example2" class="navbar sticky-top bg-dark" style="top: 110px;">
        ...
        <div class="col-auto me-1">
            ...
            <a href="{{ url('paymentMode') }}" class="btn btn-info" title="Back"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-left"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg></a>
...
@if (Request::path() == 'paymentMode')
    <div class="table-responsive mt-2" id="add">
        <table id="landingPageBrowser3SIS" class="table dt-table-hover dataTable no-footer {{ theme('table') }}">
            <thead>
                <tr>
                    <th title="Payment Mode Id">ID</th>
                    <th>Payment Name</th>
                    <th>User</th>
                    <th>Action</th>
                ...
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                        <div class="row g-3">
                            ...
                            @endif
                            <div class="col-md-6">
                                <label for="inputEmail4" class="form-label">Payment Mode Id<b class="text-danger">*</b></label>
                                <input type="text" name='BMPMHPaymentModeId' id='BMPMHPaymentModeId'
                                    ...
                                    value="{{ old('BMPMHPaymentModeId', $edit_data['BMPMHPaymentModeId'] ?? '') }}">
                            </div>
                            <div class="col-md-6">
                                <label for="inputPassword4" class="form-label">Description 1<b class="text-danger">*</b></label>
                                <input type="text" name='BMPMHDesc1' id='BMPMHDesc1'
                                    class='form-control threshold' maxlength="20"
                                    placeholder="Enter Description 1"
                                    style='border-color: rgb(102, 175, 233); outline: 0px'
                                    value="{{ old('BMPMHDesc1', $edit_data['BMPMHDesc1'] ?? '') }}">
                            </div>
                            <div class="col-12">
                                <label for="inputAddress" class="form-label">Description 2</label>
                                <textarea name='BMPMHDesc2' id='BMPMHDesc2' class='form-control textarea' maxlength="200"
                                    placeholder="Enter Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('BMPMHDesc2', $edit_data['BMPMHDesc2'] ?? '') }}</textarea>
                            </div>
                        </div>
                        </form>
                    </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" name='BMPMHUser' id='BMPMHUser'
                                    ...
                                    value="{{ old('BMPMHUser', $edit_data['BMPMHUser'] ?? '') }}" readonly>
                                     @else
                                      <input type="text" name='BMPMHUser' id='BMPMHUser'
                                    class='form-control input_border'
                            ...
                            <div class="col-md-4">
                                <label for="Created" class="form-label">Created On</label>
                                 @if(!empty($edit_data['id']))
                                   <input type="text" name='BMPMHLastCreated' id='BMPMHLastCreated'
                                    class='form-control input_border'
                                    value="{{ date('d-m-Y',strtotime($edit_data['BMPMHLastCreated']))}}" readonly>
                                 @else
                                  <input id="BMPMHLastCreated" name='BMPMHLastCreated' class="form-control input_border" type="text" placeholder="Select Date.." readonly="readonly">
                                @endif
                            </div>
                            <div class="col-md-4">
                                <label for="Updated" class="form-label">Updated On</label>
                                 @if(!empty($edit_data['id']))
                                  <input id="BMPMHLastUpdated" name='BMPMHLastUpdated' class="form-control input_border" type="text" placeholder="Select Date.." readonly="readonly" value="{{ date('d-m-Y',strtotime($edit_data['BMPMHLastUpdated']))}}">
                                 @else
                                  <input id="BMPMHLastUpdated" name='BMPMHLastUpdated' class="form-control input_border" type="text" placeholder="Select Date.." readonly="readonly">
                                @endif
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#BMPMHPaymentModeId,#BMPMHDesc1').removeClass(
            'border border-danger');

        if ($('#BMPMHPaymentModeId').val() == '') {
            $('#BMPMHPaymentModeId').addClass('border border-danger');
        }
        if ($('#BMPMHDesc1').val() == '') {
            $('#BMPMHDesc1').addClass('border border-danger');

        }
        // if($('#BMPMHDesc2').val() == ''){
        // $('#BMPMHDesc2').addClass('border border-danger');
        // }
        $('.msg_error').html('');
        if ($('#BMPMHPaymentModeId').val() == '' || $('#BMPMHDesc1').val() == '') {
            if ($('#BMPMHPaymentModeId').val() == '') {
                $('.msg_error').append('<p>Please Enter Payment Mode name !</p>');
            }
            if ($('#BMPMHDesc1').val() == '') {
                $('.msg_error').append('<p>Please Enter Payment Mode Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        if (action == 'insert') {
                            $finalMessage3SIS = fnSingleLevelFinalSave('paymentMode', $('#BMPMHPaymentModeId')
                                .val(), $('#BMPMHDesc1').val(), 'Added');
                        ...
                        if (action == 'update') {
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: 'paymentMode update successfully',
                                allowOutsideClick: false,
                                timer: 5000,
                            })
                            window.location = "{{ url('paymentMode') }}";

                        }
                    }
                    if (response.status == 'error') {
                        $('.error_msg').append('<p>Payment Mode Master not save</p>');
                    }
                }
            })
        }
    ...
    $('#Undelete_Data').click(function() {
        ...
        $('.modal-title').text('Restore Payment Mode');
    ...
    $(document).on('click', '.delete', function() {
        ...
        $deleteMessage3SIS = fnConfirmationMsg('Delete', 'Payment Mode', id, desc);
        $successMessage3SIS = fnSuccessMsg('Deleted', 'Payment Mode', id, desc);
    ...
    $(document).on('click', '.restore', function() {
        ...
        var id = $(this).attr('id');
        var desc = $(this).closest("tr").find("td:eq(1)").text();
        $restoreMessage3SIS = fnConfirmationMsg('Restore', 'Payment Mode', id, desc);
        Swal.fire({
